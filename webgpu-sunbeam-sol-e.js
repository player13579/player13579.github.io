...
@fragment fn fs_main(@builtin(position) frag:vec4f) -> @location(0) vec4f {
  ...
  let terminal=1.0-0.48*smoothstep(0.76,1.0,u);
  ...
  let tail=1.0-smoothstep(endU-0.08,endU+0.003,u);
  let finite=head*tail;
  let last=1.0-smoothstep(0.84,1.0,u);
  ...
  let supply=(bell(palmD,0.82)*0.30+palmCone*0.74)*p.phase.x;
  let bulk=(1.0-smoothstep(0.68,1.08,lateral))*finite*last*p.phase.z;
  let dense=bell(lateral,0.47)*finite*last*p.phase.z;
  let caustic=bell((center/widthPx)-0.35*sin(u*12.0-p.state.x*15.0),0.31);
  let inner=(0.30+0.70*caustic)*dense;
  let shell=bell(lateral-0.93,0.19)*finite*last*p.phase.w;
  ...
  let outflow=bell(endD,0.72)*p.phase.z*growth*0.18;
  let obs=bell(lateral,1.39)*finite*last*p.state.y*0.115*p.phase.z;
  ...
  let C=(bulk*0.40+dense*0.40+inner*0.26+outflow)*coreOn;
  let B=refracted*boundaryOn;
  let O=obs*obsOn;
  let n=max(1.0,p.control.x);
  let overlap=1.0/sqrt(n);
  let coverage=clamp((S*0.34+C*0.46+B*0.13)*overlap,0.0,0.72);
  let solar=vec3f(1.0,0.59,0.16);
  let pale=vec3f(1.0,0.88,0.50);
  let hot=vec3f(1.0,0.96,0.78);
  let emission=(solar*(S*0.73+B*0.52)+pale*C*0.57+
    hot*(inner*0.31*coreOn+S*0.19)+solar*O)*overlap;
