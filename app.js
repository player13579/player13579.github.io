...
const DVA_CLIENT_RELEASE = "switch-drag-listbox-next-tick-identity-v605";
...
const GENERATED_ITEM_TEXTURE_CACHE_VERSION = "switch-drag-listbox-next-tick-identity-v605";
...
function validateOpenSwitchDragMenu() {
  ...
  const descriptor = switchDragFocusTargetAvailable(gesture.source)
    ? switchDragDescriptorForSource(gesture.source)
    : null;
  const currentOptions = Array.isArray(descriptor?.options) ? descriptor.options : [];
  const ownedOptions = Array.isArray(gesture.options) ? gesture.options : [];
  const sameOptionIdentity = currentOptions.length === ownedOptions.length && currentOptions.every((option, index) =>
    String(option?.key || "") === String(ownedOptions[index]?.key || "") &&
    Boolean(option?.selected) === Boolean(ownedOptions[index]?.selected)
  );
  if (!descriptor || !sameOptionIdentity) {
    closeSwitchDragMenu();
  }
}
...
function handleFieldFeedDialogKeydown(event) {
  if (!state.fieldFeedOpen) return false;
  const eventTarget = event.target instanceof Element ? event.target : document.activeElement;
  const activeElement = document.activeElement;
  const editableTarget = eventTarget?.matches?.('input, textarea, [contenteditable="true"]')
    ? eventTarget
  ...
  if (event.key === "Escape" || event.code === "Backspace") {
    event.preventDefault();
    if (!event.repeat) setKeyboardSelection(els.chatInput, false);
    return true;
  }
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && state.screen !== "title") {
      ...
      const destination = state.screen === "tactics" && state.tacticsReturnScreen === "game" && state.data
        ? "game"
        : "title";
      if (destination === "title") void returnToTitle();
      else switchScreenWithEffect(destination);
      return;
    }
...
async function leaveCurrentRoom(options = {}) {
  if (!state.roomId || !state.playerId || state.leaveRoomInFlight) return false;
  const roomId = state.roomId;
  const playerId = state.playerId;
  const generation = state.roomSessionGeneration;
  const destination = options.destination || (state.data?.soloMission ? "tactics" : "title");
  const leaveOwner = { roomId, playerId, generation };
  state.leaveRoomInFlight = leaveOwner;
  els.leaveRoomButton.disabled = true;
  try {
    const result = await request("/api/leave", { roomId, playerId }, { quiet: true });
    if (!result || state.leaveRoomInFlight !== leaveOwner || !isCurrentRoomSession(roomId, playerId, generation)) return false;
    resetLocalSession();
    switchScreenWithEffect(destination);
    if (options.announce !== false) showToast("マッチから退出しました。");
    return true;
  } finally {
    if (state.leaveRoomInFlight === leaveOwner) state.leaveRoomInFlight = null;
...
function render() {
  ...
  if (state.phaseUiKey !== phaseUiKey) {
    closeSwitchDragMenu();
    if (data?.phase === "meeting") cancelTransientGameInputForBackground();
...
const version = "switch-drag-listbox-next-tick-identity-v605";
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker.register(new URL("sw.js?v=switch-drag-listbox-next-tick-identity-v605", document.baseURI)).then(async (registration) => {
