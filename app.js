...
function renderItemControl(data) {
  ...
  if (state.itemRenderKey !== renderKey) {
    ...
    items.forEach((item) => {
      ...
      button.addEventListener("click", () => {
        ...
        if (item.inventoryKind !== "weapon" || item.sourceId === state.data?.self?.gunnerWeapon) return;
  ...
  els.itemUseButton.hidden = selected?.inventoryKind === "weapon";
  ...
  els.itemUseButton.textContent = "使用 [Shift+V]";
...
const version = "gravity-friendly-fire-v395";
