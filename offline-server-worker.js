...
const LIMIT_BREAK_SPEED_MULTIPLIER = ACCELERATE_SPEED_MULTIPLIER * 1.44;
...
const GRAVITY_STORM_SAFE_RADIUS = WORLD_UNITS_PER_METER * 2;
const GRAVITY_STORM_DAMAGE_MIN = 0.05;
const GRAVITY_STORM_DAMAGE_MAX = 0.16;
...
const GRAVITY_STORM_KNOCKBACK_MIN = 110;
const GRAVITY_STORM_KNOCKBACK_MAX = 340;
...
const OPERATORS = {
  defender: [
    ...
    },
    {
      ...
      details: "重力による時空の曲率を操作するオペレーター。転移は1MPを消費し、地点または他人の付近へ自分を移動する。心臓転移は位置を公開して遠隔確殺する。アクセラレートとディーセラレートは1MPで8秒間、対象の行動時間を相対変化させる。理知中はリビテーションで床のない場所も移動できる。グラビティストームは前方に乱数強度の継続ダメージ・減速・重力変位を起こし、発動者の半径2mは安全地帯になる。"
    },
    {
      id: "defender-flora",
      role: "defender",
...
const SOLO_MISSIONS = Object.freeze({
  ...
  emp: Object.freeze({
    id: "emp",
    name: "EMP共鳴訓練",
    objective: "3秒周期で停止発動するBotのEMPへ同位相または逆位相EMPを重ねる",
    team: "defender",
    operatorId: "operator-fighter",
    botCount: 2,
    attackerCount: 1,
...
function updatePlayerProfiles(room) {
  ...
  for (const player of players) {
    ...
    const specialWinner = room.winner === "idea" && room.ideaWinnerId === player.id;
    const contributed = player.role === "attacker"
      ? kills >= attackerTarget || specialWinner
      : kills >= 1 || specialWinner;
    ...
    if (contributed) profile.points = Math.min(24, (Number(profile.points) || 0) + 1);
...
function convertResource(room, player, rawDirection) {
  ...
  if (direction === "mana-to-stamina") {
    ...
    player.stamina = Math.min(MAX_STORED_STAMINA, Number(player.stamina || 0) + MANA_TO_STAMINA_AMOUNT);
    player.staminaUpdatedAt = now();
    setImmediateFeedback(player, "資源変換", `MP-1 / SP+${MANA_TO_STAMINA_AMOUNT}`);
  } else if (direction === "stamina-to-mana") {
    if (availableStamina(player) < STAMINA_TO_MANA_COST) {
      throw new ApiError(400, `変換には${STAMINA_TO_MANA_COST}SPが必要です。`);
    }
    spendStamina(player, STAMINA_TO_MANA_COST);
    setMana(room, player, Number(player.mana) + 1, "MP変換");
    setImmediateFeedback(player, "資源変換", `SP-${STAMINA_TO_MANA_COST} / MP+1`);
  } else {
...
function donateCredits(room, player) {
  ...
  if (rationalDonation) {
    pushMagicEffect(room, "object-resolvePoint", player, {
      radius: 125,
      playerId: player.id,
      effectKind: "luckBoost"
    });
...
function beginIdeaAscension(room, player, timestamp) {
  ...
  player.drone.active = false;
  clearAttackState(player);
  room.ideaWinnerId = player.id;
  room.pendingIdeaVictoryAt = player.ascensionUntil;
...
function replenishStamina(entity, timestamp, allowRegen = true, multiplier = 1, room = null) {
  ...
  if (allowRegen) {
    ...
    const current = Math.min(MAX_STORED_STAMINA, Math.max(0, Number(entity.stamina) || 0));
    const restored = Math.min(recovery, MAX_STORED_STAMINA - current);
...
function effectiveMovementMultiplier(room, player, timestamp = now()) {
  const taserMultiplier = Number(player.taserSlowedUntil) > timestamp ? TASER_MOVEMENT_MULTIPLIER : 1;
...
function fighterSlash(room, player, targetId = "") {
  ...
  ensureConscious(player);
  const timestamp = now();
  // Slash has no cooldown. This field is only a client-side motion/effect window.
  player.slashActiveUntil = 0;
...
function teleportPlayer(room, player, rawX, rawY, targetId = "", mode = "body") {
  ...
  if (mode === "heart") {
    ...
    pushSound(room, "heartTeleportReveal", player, {
      ownerId: player.id,
      sourceKind: "magic",
      maxDistance: 10000,
      volume: 1
    });
    pushEvent(room, `${whichRoom(getMap(room), player)} 付近で心臓転移の発動座標を検知しました。`);
...
function useGravityStorm(room, player) {
  ...
  const zone = {
    ...
    safeX: player.x,
    safeY: player.y,
    endsAt: now() + GRAVITY_STORM_DURATION_MS,
    lastPulseAt: 0
  };
  room.gravityZones ||= [];
...
function advanceGravitySystems(room, timestamp, elapsedMs) {
  ...
  for (const zone of room.gravityZones) {
    ...
    for (const target of room.players.values()) {
      ...
      const damage = Math.round((GRAVITY_STORM_DAMAGE_MIN +
        (GRAVITY_STORM_DAMAGE_MAX - GRAVITY_STORM_DAMAGE_MIN) * severity) * 100) / 100;
      ...
      target.bodyHits = Math.round((Number(target.bodyHits || 0) + damage) * 100) / 100;
      ...
      target.gravityStormSlowMultiplier = gravitySlowWasActive
        ...
        : slowMultiplier;
      setImmediateFeedback(target, "グラビティストーム", `HP-${damage.toFixed(2)} / 移動速度${Math.round(slowMultiplier * 100)}%`);
      ...
      if (modeRoll < 0.45) {
        ...
        const amount = GRAVITY_STORM_KNOCKBACK_MIN + (GRAVITY_STORM_KNOCKBACK_MAX - GRAVITY_STORM_KNOCKBACK_MIN) * severity;
        displaceByGravity(room, target, dx, dy, amount);
        pushMagicEffect(room, "gravity-storm-blast", target, { radius: 190, playerId: zone.ownerId, targetId: target.id, variant: String(Math.round(amount)) });
      } else if (modeRoll < 0.72) {
        const amount = GRAVITY_STORM_PULL_MIN + (GRAVITY_STORM_PULL_MAX - GRAVITY_STORM_PULL_MIN) * severity;
        displaceByGravity(room, target, zone.x - target.x, zone.y - target.y, amount);
        pushMagicEffect(room, "gravity-storm-pull", target, { radius: 175, playerId: zone.ownerId, targetId: target.id });
      } else if (modeRoll < 0.90) {
        const angle = Math.random() * Math.PI * 2;
        const amount = GRAVITY_STORM_KNOCKBACK_MIN + (GRAVITY_STORM_KNOCKBACK_MAX - GRAVITY_STORM_KNOCKBACK_MIN) * severity * 0.72;
        displaceByGravity(room, target, Math.cos(angle), Math.sin(angle), amount);
        pushMagicEffect(room, "gravity-storm-heavy", target, { radius: 155, playerId: zone.ownerId, targetId: target.id, variant: String(Math.round(slowMultiplier * 100)) });
      } else {
        ...
        target.vx = 0;
        target.vy = 0;
        pushMagicEffect(room, "gravity-storm-crush", target, { radius: 165, playerId: zone.ownerId, targetId: target.id, variant: String(pinMs) });
      }
    }
  }
}
...
function triggerSubstitution(room, player, reason, timestamp = now()) {
  if (player.limitBreakActive || player.substitutionCharges <= 0 || !passivesEnabled(player) || !itemStorageAvailable(player, timestamp)) return false;
...
function transferOwnedResource(room, player, targetId, itemId, rawAmount, credits = false) {
  ...
  if (credits) {
    ...
    const item = removeTransferableItem(player, String(itemId || ""), amount);
...
function resolveStandardEmp(room, pulse, timestamp) {
  ...
  for (const target of room.players.values()) {
    if (target.id !== player.id && target.alive && !target.ejected && distance(pulse, target) <= EMP_RANGE) {
      ...
      } else if (hackerEmpOpeningProtected(room, target, timestamp)) {
        continue;
      } else {
...
function resolveEmpInteraction(room, first, second, timestamp) {
  if (room.soloMission?.id === "emp" && [first.playerId, second.playerId].includes(room.soloMission.playerId)) {
    room.soloMission.empResonated = true;
  }
  ...
  if (!samePhase) {
    ...
    pushEvent(room, "正相EMPと逆相EMPが干渉し、互いに相殺されました。");
    touch(room);
    return;
  }
...
function activateEmp(room, player, rawPhase = "positive") {
  ...
  player.empReadyAt = timestamp + EMP_COOLDOWN_MS;
  room.activeEmps ||= [];
...
function recoverMapObjectStatuses(room, player, source) {
  const hadStatus = Boolean(
    ...
    Number(player.taserSlowedUntil) > 0 || Number(player.abilityDisabledUntil) > 0 ||
...
function applyBottleShardSplash(room, player, itemId, center, level = 0) {
  ...
  for (const target of room.players.values()) {
    ...
    if (lethal) destroyPlayerUnconditionally(room, null, target, "瓶の破片");
  }
...
function applyThrownImpactDamage(room, source, landing, label, damage, radius) {
  ...
  if (Number(target.overheal) > 0) {
    ...
    target.bodyHits = Math.round((Math.max(0, Number(target.bodyHits) || 0) + Math.max(0.1, Number(damage) || 0.1)) * 100) / 100;
  }
  const lethal = Number(target.bodyHits) >= 2;
  pushHitEffect(room, target, "body", lethal);
  if (lethal) destroyPlayerUnconditionally(room, source, target, `${label}の衝撃`);
  return true;
}
...
function resolveThrownOwnedLanding(room, source, thrown, landing) {
  ...
  const damage = isInvention ? 1.05 + level * 0.2 : isWeapon ? 0.62 + level * 0.15 : 0.45 + level * 0.12;
  const radius = isInvention ? 120 + level * 16 : isWeapon ? 82 + level * 12 : 72 + level * 10;
...
function useInventoryItem(room, player, itemId, rawHoldMs = 0) {
  ...
  if (itemId === "mineral-water") {
    ...
    applyPersistentStatus(room, player, player, "poison", strength);
    setImmediateFeedback(player, "有害物質曝露", `${definition.label} / 毒強度${strength.toFixed(2)}`);
  } else if (itemId === "molotov" || itemId === "heated-water") {
    const strength = 1 + level * 0.4;
    applyPersistentStatus(room, player, player, "burn", strength);
...
function useFireJutsu(room, player, rawHoldMs = 0) {
  if (room.phase !== "playing" || !player.alive || player.ejected || player.inVent) {
...
function floraSunbeam(room, player, converged = false, targetId = "", direction = {}) {
  ...
  const selected = converged ? (tracked ? [{ target: tracked, along: distance(player, tracked) }] : candidates.slice(0, 1)) : candidates;
  let hits = 0;
  ...
  const end = selected[0]?.target || { x: player.x + dx * SUNBEAM_RANGE, y: player.y + dy * SUNBEAM_RANGE };
...
function activateLimitBreak(room, player) {
  if (room.phase !== "playing" || !hasOperatorAccess(player, "fighter")) {
    throw new ApiError(403, "ファイターだけがリミットブレイクを発動できます。");
  }
  if (!player.alive || player.ejected || player.inVent) throw new ApiError(403, "現在は発動できません。");
  ensureAbilityAvailable(player);
  if (player.limitBreakActive) {
    stopLimitBreak(room, player, "任意解除されました");
    pushMagicEffect(room, "limit-break", player, { radius: 110, playerId: player.id, variant: "release" });
    touch(room);
    return;
  }
  spendOperatorMana(room, player, "リミットブレイク");
...
function recoverHackerTargetStatus(room, player, targetId) {
  ...
  const timestamp = now();
  const timerFields = [
    "itemDisabledUntil", "slowedUntil", "taserSlowedUntil",
...
function deleteHackerTargetHp(room, player, targetId) {
  ...
  return destroyPlayerUnconditionally(room, player, target, "バイブコーディング: HP削除");
}
...
function destroyPlayerUnconditionally(room, source, target, reason, options = {}) {
  ...
  pushEvent(room, `${target.name} は${reason}で破壊されました。`);
  return true;
}
...
function useAlchemistInvention(room, player, invention) {
  if (room.phase !== "playing" || player.special !== "alchemist" || !player.alive || player.ejected || player.inVent) {
  ...
  if (id === "computer") {
    ...
    const targets = inventionLineTargets(room, player, Math.max(getMap(room).width, getMap(room).height) * 2, Math.max(getMap(room).width, getMap(room).height), true);
    for (const { target } of targets) destroyPlayerUnconditionally(room, player, target, "エクスカリバー");
    pushMagicEffect(room, "alchemy-excalibur", player, { radius: 900, playerId: player.id, variant: "forward-half-map" });
    checkWin(room);
    if (!(room.phase === "ended" && room.winner === "attackers" && player.role === "attacker")) {
      destroyPlayerUnconditionally(room, player, player, "エクスカリバーの代償");
    }
  } else if (id === "railgun") {
    for (const { target } of inventionLineTargets(room, player, 5000, 38, false)) destroyPlayerUnconditionally(room, player, target, "レールガン");
    pushMagicEffect(room, "alchemy-railgun", player, { radius: 100, targetX: player.x + player.aimX * 5000, targetY: player.y + player.aimY * 5000, playerId: player.id });
  } else if (id === "particle-cannon") {
    player.particleCannonUntil = now() + 6_000;
    player.particleCannonNextAt = 0;
    pushMagicEffect(room, "alchemy-particle-cannon", player, {
      radius: 150,
      targetX: player.x + player.aimX * 1250,
      targetY: player.y + player.aimY * 1250,
      playerId: player.id,
      variant: "continuous"
    });
  }
  ...
  pushEvent(room, `${player.name} が素敵な発明品 ${id} を使用しました。`);
  checkWin(room);
  touch(room);
}
...
function advanceParticleCannon(room, player, timestamp) {
  ...
  if (targets[0]) destroyPlayerUnconditionally(room, player, targets[0].target, "荷電粒子砲");
  pushMagicEffect(room, "alchemy-particle-beam", player, {
    radius: 140, targetX: player.x + player.aimX * 1250, targetY: player.y + player.aimY * 1250, playerId: player.id
  });
}
...
function useBorrowedAbility(room, player, type, options = {}) {
  ...
  if (key === "fighter") {
    activateLimitBreak(room, player);
  } else if (key === "gravity") {
...
function resolvePendingAttack(room, player, timestamp = now()) {
  ...
  try {
    ...
    const hitZone = player.limitBreakActive || usePush || Math.random() < QUICK_ATTACK_LETHAL_CHANCE ? "head" : "body";
...
function killPlayer(room, killer, targetId, options = {}) {
  ...
  if (!ignoreDodge && target.dodgeActiveUntil > timestamp) {
    ...
    if (hasOperatorAccess(target, "fighter") && passivesEnabled(target)) {
      ...
      pushEvent(room, `${target.name} が回避に成功し、攻撃者 ${killer.name} を返り討ちにしました。`);
      checkWin(room);
      touch(room);
    ...
    pushEvent(room, `${target.name} が ${killer.name} のキルを回避しました。`);
    touch(room);
    return "dodged";
  }

  if (hitZone === "head" && !target.limitBreakActive && itemStorageAvailable(target, timestamp) && passivesEnabled(target) && target.gritCharges > 0) {
...
function clearShotPath(room, shooter, target, directionX, directionY) {
  ...
  for (let step = 1; step < steps; step += 1) {
    ...
    if ((room.alchemyObjects || []).some((object) => object.type === "cover" && (!object.endsAt || object.endsAt > now()) && Math.hypot(x - object.x, y - object.y) <= object.radius)) return false;
  }
  return true;
}
...
function findGunnerTarget(room, shooter, weapon, dx, dy) {
  return [...room.players.values()]
...
function fireGunnerRound(room, shooter, weapon, timestamp) {
  ...
  if (targetEntry) {
    ...
    if (weapon.id === "taser" && outcome !== "lethal" && targetEntry.player.alive) {
...
function useGunnerHeavyWeapon(room, player) {
  ...
  if (weapon === "rpg") {
    for (const target of targets.filter((candidate) => distance(player, candidate) <= 300)) {
      try {
        killPlayer(room, player, target.id, {
          ranged: true, hitZone: "body", damage: 1, ignoreRange: true,
          allowAnyKiller: true, targetRole: target.role, magic: true
        });
      } catch {}
    }
    ...
    if (target) {
      try {
        killPlayer(room, player, target.id, {
          ranged: true, hitZone: "head", ignoreRange: true,
          allowAnyKiller: true, targetRole: target.role, magic: true
        });
      } catch {}
      pushMagicEffect(room, "gunner-missile", player, { radius: 150, playerId: player.id, targetId: target.id, targetX: target.x, targetY: target.y });
    }
  }
  awardAbilityContribution(player, 1);
...
function jumpPlayer(room, player, rawDx, rawDy) {
  ...
  const motionDuration = Math.max(320, Math.min(1800, 260 + distanceToJump * 0.72));
  player.x = destination.x;
  player.y = destination.y;
...
function resultRanking(room) {
  const entries = [...room.players.values()]
    .map((player) => ({
      ...
      contributionScore: (Number(player.totalKills) || 0) * KILL_CONTRIBUTION,
      luminousSuccess: Boolean(player.luminousActive)
    }))
    .sort((a, b) => (
      b.contributionScore - a.contributionScore ||
      b.actualKills - a.actualKills ||
      Number(b.luminousSuccess) - Number(a.luminousSuccess) ||
  ...
  return {
    ...
    self: {
      ...
      abilityCosts: {
        ...
        limitBreak: LIMIT_BREAK_MANA_COST,
    ...
    soloMission: (() => {
      ...
      return {
        ...
        hint: room.soloMission.hintUnlocked
          ? "CPUは自分へアクセラレート→練気4回→再度アクセラレート→練気4回→心臓転移を繰り返します。練気中の行動不能時間か、心臓転移前の位置公開を狙って妨害してください。"
          : ""
      };
    })(),
    winner: room.winner,
...
async function handleApi(req, res) {
  ...
  switch (pathname) {
    ...
    case "/api/fighter-slash": {
      const { room, player } = requireRoomPlayer(body);
      fighterSlash(room, player, String(body.targetId || ""));
      payload = serialize(room, player);
      break;
    }
    ...
    case "/api/fire-jutsu": {
      const { room, player } = requireRoomPlayer(body);
      useFireJutsu(room, player, body.holdMs);
      payload = serialize(room, player);
      break;
    }
    ...
    case "/api/item-use": {
      const { room, player } = requireRoomPlayer(body);
      useOwnedItem(room, player, String(body.itemId || ""), body.holdMs);
      payload = serialize(room, player);
      break;
    }
    ...
    case "/api/alchemist-invention": {
      const { room, player } = requireRoomPlayer(body);
      useAlchemistInvention(room, player, body.invention);
      payload = serialize(room, player);
      break;
    }
...
function botManaTarget(bot) {
  if (bot.special === "fighter") return LIMIT_BREAK_MANA_COST;
