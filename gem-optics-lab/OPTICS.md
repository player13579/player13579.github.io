...
6. 回転中・静止中ともに380–780 nmの24波長、最高精細では48波長を別々に追跡します。CIE 1931等色関数のWyman/Sloan/Shirley近似を使い、線形XYZの空間で積算します。
...
各ピクセルで3波長ずつ循環して追跡し、波長の完全な周期が揃った時点で表示を更新します。回転・ドラッグ・拡大縮小中も、選択中の波長数と内部境界の上限を維持します。一つの表示画像は同じ視点で計算し、異なる視点の履歴を混ぜません。静止後はサブピクセル位置をずらした積算を続け、輪郭をさらに滑らかにします。波長と位置の周期の相関を弱めています。

全波長の計算を小さな処理単位に分けて実行するため、端末や品質設定によって回転中の更新間隔が変わります。
