...
4. 内部から出る光を各境界で加算し、残りの反射光を回転中は最大8・12・20回、静止中は最大16・24・40回の内部境界イベントまで追跡します。臨界角 `asin(1/n)` を超えると全反射します。
5. 内部の移動距離ごとに、Beer–Lambert 則 `T = exp(−σd)` で吸収を適用します。
6. 380–780 nmの可視光を、回転中は6・12・24波長、静止中は24・48波長で追跡します。CIE 1931等色関数のWyman/Sloan/Shirley近似を使い、線形XYZの空間で積算します。
...
タブレット推奨は、実際に完成した動画像の更新時間を測り、約30fpsを目標に描画解像度を調整します。解像度変更には待ち時間と幅を設け、頻繁な画質の変動を抑えます。端末のGPU性能や温度によって到達する速度は異なります。停止後の積算が完成するとGPU描画を止め、非表示のページでも描画を休止します。

標準設定の画素数は回転開始時に約20万、回転中は約8万〜30万、停止後は最大約45万です。6波長でも基準の白色が偏らないよう、各波長帯の中でD65と等色関数の積を数値積分し、帯域の代表光路に重みを与えています。Yを基準に共通の正規化を行い、XYZ成分ごとに別々の白合わせはしていません。広い波長帯の細かな光路差は省略するため、回転中の虹色の細部は静止後より粗くなります。
