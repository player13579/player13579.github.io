...
4. 内部から出る光を各境界で加算し、残りの反射光を回転中は最大12・16・20回、静止中は最大24・28・40回の内部境界イベントまで追跡します。臨界角 `asin(1/n)` を超えると全反射します。
5. 内部の移動距離ごとに、Beer–Lambert 則 `T = exp(−σd)` で吸収を適用します。
6. 380–780 nmの可視光を、回転中は12・24波長、静止中は24・48波長で追跡します。CIE 1931等色関数のWyman/Sloan/Shirley近似を使い、線形XYZの空間で積算します。
...
| 品質 | 回転中の波長数 / 境界上限 | 静止中の波長数 / 境界上限 | 静止中の積算サンプル数 |
| --- | ---: | ---: | ---: |
| iPad高画質・自動（標準） | 12 / 16 | 48 / 28 | 96 |
| バランス（固定） | 12 / 12 | 24 / 24 | 64 |
| 最高精細（PC向け） | 24 / 20 | 48 / 40 | 128 |

各ピクセルで3波長ずつ循環して追跡し、選択中の波長の完全な周期が揃った時点で表示を更新します。回転・ドラッグ・拡大縮小中は表の軽い設定を使い、同じ視点で計算して異なる視点の履歴を混ぜません。宝石のファセット形状は共通です。静止後は波長数・追跡回数・解像度を上げ、サブピクセル位置をずらした積算で輪郭を滑らかにします。波長と位置の周期の相関を弱めています。

iPad高画質は、実際に完成した動画像の更新時間を測り、約24fpsを目安に描画解像度を調整します。速い場面では上限まで画素数を増やし、遅い場面では減らします。約38〜52msの範囲では解像度を保ちます。解像度変更には待ち時間と幅を設け、頻繁な画質の変動を抑えます。端末のGPU性能や温度によって到達する速度は異なります。停止後の積算が完成するとGPU描画を止め、非表示のページでも描画を休止します。

標準設定の画素数は回転開始時に約42万、回転中は約26万〜120万、停止後は最大約160万です。Retina画面も端末の画素比2まで使い、画素数の予算で上限を制御します。少ない波長数でも基準の白色が偏らないよう、各波長帯の中でD65と等色関数の積を数値積分し、帯域の代表光路に重みを与えています。Yを基準に共通の正規化を行い、XYZ成分ごとに別々の白合わせはしていません。広い波長帯の細かな光路差は省略するため、回転中の虹色の細部は静止後より粗くなります。
